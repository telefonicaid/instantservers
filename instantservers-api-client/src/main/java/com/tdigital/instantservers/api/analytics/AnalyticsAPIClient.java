...
/**
 * Cloud Analytics API (CaAPI)
 */
public class AnalyticsAPIClient extends BaseAPIClient {

    /**
     * CaAPI uses a specific X_API_VERSION header
     */
    private static final String JOYENT_CA_X_API_VERSION = "ca/0.1.8";

    public AnalyticsAPIClient(String login, String password, String endpoint) {
        super(login, password, endpoint, JOYENT_CA_X_API_VERSION);
    }

    public AnalyticsSchema describeAnalytics() throws InstantServersApiException {
        return get("/ca", AnalyticsSchema.class).getValue();
    }

    public Instrumentation[] listInstrumentations() throws InstantServersApiException {
        return get("/ca/instrumentations", Instrumentation[].class).getValue();
    }

    public Instrumentation createInstrumentation(String module, String stat) throws InstantServersApiException {
        return post("/ca/instrumentations?module={module}&stat={stat}", null, Instrumentation.class, null, module, stat).getValue();
    }

    public Instrumentation createInstrumentation(String module, String stat, String decomposition) throws InstantServersApiException {
        return post("/ca/instrumentations?module={module}&stat={stat}&decomposition={decomposition}", null, Instrumentation.class, null, module, stat, decomposition).getValue();
    }

    public Instrumentation createInstrumentation(Instrumentation input) throws InstantServersApiException {
        ...
        try {
            ...
            throw new IOError(e); // not recoverable
        }

        String[] params = parameters.toArray(new String[parameters.size()]);
        return post("/ca/instrumentations?" + queryString.substring(1), null, Instrumentation.class, null, params).getValue();
    }

    public SimpleInstrumentationValue getInstrumentationValue(String instrumentationId) throws InstantServersApiException {
        return get("/ca/instrumentations/{instrumentationId}/value/raw", SimpleInstrumentationValue.class, instrumentationId).getValue();
    }

    public DecompositionInstrumentationValue getDecompositionInstrumentationValue(String instrumentationId) throws InstantServersApiException {
        return get("/ca/instrumentations/{instrumentationId}/value/raw", DecompositionInstrumentationValue.class, instrumentationId).getValue();
    }

    public void deleteInstrumentation(String instrumentationId) throws InstantServersApiException {
        delete("/ca/instrumentations/{instrumentationId}", instrumentationId);
    }
}
